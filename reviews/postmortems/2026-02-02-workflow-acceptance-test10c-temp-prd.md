...
- Smallest increment: change test 10c PRD path to `plans/.tmp/valid_prd_10c_non_promo.json`.
...
- Response: require acceptance tests to write temporary fixtures under a non-ignored temp dir (for example `plans/.tmp/`) rather than tracked fixtures; require any acceptance test that commits to ensure a staged diff exists before `git commit`.
