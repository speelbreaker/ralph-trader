...
## 1) Constraint (ONE)
- How it manifested (2-3 concrete symptoms): parallel workflow_acceptance workers exited during test 10c and test 16; git commit failed because the PRD path was ignored and the dummy test file already existed; speedup timing comparison was blocked.
- Time/token drain it caused: ~28 minutes per failed run before the stop.
- Workaround I used this PR (exploit): moved the test 10c PRD to a non-ignored temp path and made test 16 use a unique dummy test file passed to the delete stub.
...
- Smallest increment: change test 10c PRD path to `plans/.tmp/valid_prd_10c_non_promo.json` and use a unique dummy test filename for test 16.
...
- Response: require acceptance tests to write temporary fixtures under a non-ignored temp dir (for example `plans/.tmp/`) rather than tracked fixtures; require any acceptance test that commits to ensure a staged diff exists before `git commit`, and use unique filenames for test artifacts that might already exist in the repo.
