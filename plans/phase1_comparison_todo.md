...
| RALPH-CMP-012 | open | 20260213_223424_slice6_compare | `S6-006` crash-mid-intent evidence is single-scenario (1 test) versus multi-scenario matrix used in opus (7 tests), reducing failure-mode coverage breadth. | Keep the current canonical test, then add targeted restart variants (sent/acked/terminal/WAL-failure/fsync) as deterministic subtests. | `/Users/admin/Desktop/ralph/crates/soldier_infra/tests/test_crash_mid_intent.rs`; `/Users/admin/Desktop/opus-trader/crates/soldier_infra/tests/test_crash_mid_intent.rs` |
