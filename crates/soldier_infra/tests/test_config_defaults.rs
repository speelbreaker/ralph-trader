...
use soldier_infra::config::{
    ...
    MM_UTIL_KILL_DEFAULT, ParamKind, SafetyConfigInput, apply_defaults, resolve_required_f64,
    resolve_required_u64,
};
...
/// GIVEN a mismatched Appendix A default type
/// WHEN the parameter is missing
/// THEN the gate fails closed with a deterministic type error.
#[test]
fn test_type_mismatch_fails_closed() {
    let err = resolve_required_u64("mm_util_kill", None)
        .expect_err("type mismatch must fail closed for safety-critical key");
    ...
    assert!(
        ...
        "error MUST identify the key and expected type"
    );
    assert_eq!(
        err.to_string(),
        "type mismatch for safety-critical config value: mm_util_kill (expected u64)",
        "error message MUST be deterministic"
    );
}
